...
export default function AppointmentCard({ appointment }: AppointmentCardProps) {
  ...
  const canCancel = ["PENDING", "CONFIRMED"].includes(appointment.status) && start > new Date();
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {appointment.service.description && (
            <p className="mt-1 text-sm text-muted-foreground">{appointment.service.description}</p>
          )}
        </div>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <p className="mt-1 font-semibold">{appointment.service.duration} perc</p>
        </div>

        <div>
          <p className="text-sm text-gray-500">Ár</p>

          <p className="mt-1 font-semibold">{appointment.price.toLocaleString("hu-HU")} Ft</p>
        </div>
      </div>
      ...
      <div className="mt-6 border-t border-gray-100 pt-5">
        ...
        <p className="mt-1 text-sm text-gray-700">{appointment.customerNote || "Nincs megadva."}</p>
      </div>
